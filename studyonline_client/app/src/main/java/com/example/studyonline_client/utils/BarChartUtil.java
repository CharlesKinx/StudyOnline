...
public class BarChartUtil {
    ...
    private void initView(int length){
        ...
        barChart.getXAxis().setAxisMaximum(length);   //X轴最大数值
        barChart.getXAxis().setAxisMinimum(0);   //X轴最小数值
        //X轴坐标的个数    第二个参数一般填false     true表示强制设置标签数 可能会导致X轴坐标显示不全等问题
        //barChart.getXAxis().setGranularity(1);
        barChart.getXAxis().setLabelCount(length,false);
    ...
    public void showBarChart(ArrayList<ScoreInfo> myScore,ArrayList<ScoreInfo> avgScore){
        setData(myScore,avgScore);
        initView(myScore.size());
    }

    private void setData(ArrayList<ScoreInfo> myScore,ArrayList<ScoreInfo> avgScore){
        list=new ArrayList<>();
        list2=new ArrayList<>();
        labels = new ArrayList<>();

        for (int i =0;i<myScore.size();i++){
            labels.add(myScore.get(i).getCourseName());
            list.add(new BarEntry(i,avgScore.get(i).getScore()));
            list2.add(new BarEntry(i,myScore.get(i).getScore()));
        }
